...
class Solution:
    def batch_norm(self, x: List[List[float]], gamma: List[float], beta: List[float],
        ...
        if training:
            mub = np.mean(x,0)
            varb = np.var(x,0)
            xo = (x-mub)/np.sqrt(varb+eps)
            running_mean = (1-momentum)*running_mean + momentum*mub
            running_var = (1 -momentum)*running_var + momentum*varb
        else:
            xo = (x-running_mean) / np.sqrt(running_var+eps)
        out = xo*gamma + beta
